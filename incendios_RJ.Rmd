...
meteo_rj <- meteo %>%
    ...
    mutate(mes = format(.[["Data"]], "%m"), ano = format(.[["Data"]], "%Y"))
...
rm("meteo", "meteo_rj_imputado", imp_merged, ini, freq_table)
```
...
p2 <- qplot(focos_rj$Latitude, binwidth = 0.2, xlab = "Latitude")
p3 <- qplot(focos_rj$Longitude, binwidth = 0.2, xlab = "Longitude")
multiplot(p1,p2,p3)
```

```{r heatmap focos, message=FALSE}
BB <- c(min(focos_rj$Longitude), min(focos_rj$Latitude), max(focos_rj$Longitude), max(focos_rj$Latitude))
...
figure + 
    ...
    bins =9, geom = "polygon") + 
      scale_fill_gradientn(colors = heat.colors(10), guide = FALSE) + 
    scale_alpha(range = c(0.1, 0.8), guide = FALSE)
```

## Meteo

Variação da temperatura ao longo do ano por estação.
```{r AED meteo}
ggplot(data = meteo_rj, aes(x = mes, y = TempBulboSeco)) +
      geom_boxplot(color = "#189ad3", fill = "#189ad3") + 
      facet_grid(. ~ Estacao) + 
