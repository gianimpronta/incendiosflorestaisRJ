...
multi_algo_training <- function(models)
{
...
```{r }
qplot(primeira_selecao$Time) + ggtitle("Tempo dos algoritmos")
...
Eliminando 20% dos algoritmos
```{r }
qplot(primeira_selecao[primeira_selecao$Time < 20,4]) + ggtitle("Tempo dos algoritmos")
segunda_selecao <- primeira_selecao[primeira_selecao$Time < 20 & 
...
```{r Terceira sele;ão}
qplot(segunda_selecao[segunda_selecao$Time < 5,4]) + ggtitle("Tempo dos algoritmos")
terceira_selecao <- segunda_selecao[segunda_selecao$Time < 2 & 
                                      segunda_selecao$Accuracy > 0.8, ]
...
selecaofinal <- terceira_selecao[terceira_selecao$Accuracy > 0.82,]
nrow(selecaofinal)
```

Conseguimos refinar até uma lista de 43 algoritmos rápidos e eficientes, aagora vamos testar novamente com o datas e desbalanceado para reformar ainda mais a lista.

```{r Segundo teste}
r3 <- multi_algo_training(selecaofinal$Method)
```
...
No under-sampling selecionamos todas as observações da classe com menor número de observações e selecionamos aleatoriamente uma quantidade igual das observações da outra classe. O problema
```{r under-sampling}
n_sim <- table(dt_final$incendio)[2]
idx_nao <- which(dt_final$incendio == "nao")
idx_sim <- which(dt_final$incendio == "sim")
idx <- idx_nao[sample(length(idx_nao), n_sim)]
dt_nao <- dt_final[idx]
dt_sim <- dt_final[idx_sim]
dt_balanceado <- rbind(dt_sim, dt_nao)
```
